...
class ReportSaleController extends Controller
{
    public function reportView()
    {
        ...
        return view('reportSale', compact('user'));
    }
    ...
    public function downloadPDF(Request $request)
    {
        ...
        if ($start_date && $end_date) {
            $saleData = $saleData->whereBetween('sale_date', [$start_date, $end_date]);
        }
        ...
        $totalAmount = $saleData->sum('total_amount');
        ...
        foreach ($saleData as $sale) {
            $html .= '<tr style="text-align: center;">' . 
                        ...
                        '<td style="padding: 8px; border: 1px solid #ddd;">' . $sale->sale_date . '</td>' . 
                        '<td style="padding: 8px; border: 1px solid #ddd;">' . number_format($sale->total_amount, 2) . '</td>' . 
                    '</tr>';
        }
        ...
        $html .= '</tbody>' . 
            '<tfoot>' . 
                '<tr>' . 
                    '<td colspan="3" style="text-align: right; font-weight: bold; padding: 8px; border: 1px solid #ddd;">Grand Total:</td>' . 
        ...
        if ($start_date && $end_date) {
            $html .= '<div style="margin-left: 20px; margin-top: 15px; font-size: 14px;">' . 
        ...
        return $dompdf->stream('sale-' . $dateNow . '.pdf');
    }
}
