...
<body>
    <div class="header-info">
        <p><strong>K STORE</strong></p>
        <p>Alamat: Jalan Caia Blok K</p>
        <p>Tanggal Print: {{ date('d-m-Y') }}</p>
    </div>
    <div class="form-group">
        <p align="center"><b>Report Sale Data</b></p>
        <table class="static" align="center" rules="all" border="1px">
            ...
            <tbody>
                @foreach ($printSale as $item)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $item->saleID }}</td>
                        <td>{{ $item->sale_date }}</td>
                        <td>{{ $item->customer ? $item->customer->customer_name : 'General Customer' }}</td>
                        <td>{{ $item->cashier_name }}</td>
                        <td>Rp {{ number_format($item->total_amount, 2) }}</td>
                        <td>Rp {{ number_format($item->paid_amount, 2) }}</td>
                        <td>Rp {{ number_format($item->change, 2) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
